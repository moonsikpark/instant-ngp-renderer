...
struct RaysNerfSoa {
	...
	void copy_from_other_async(const RaysNerfSoa& other, cudaStream_t stream) {
		...
		CUDA_CHECK_THROW(cudaMemcpyAsync(depth, other.depth, size * sizeof(Eigen::Array4f), cudaMemcpyDeviceToDevice, stream));
		CUDA_CHECK_THROW(cudaMemcpyAsync(payload, other.payload, size * sizeof(NerfPayload), cudaMemcpyDeviceToDevice, stream));
	}
#endif

	void set(Eigen::Array4f* rgba, Eigen::Array4f* depth, NerfPayload* payload, size_t size) {
		this->rgba = rgba;
